...
**Gmail SMTP "Network is unreachable" / slow email fix.** Render's free network
advertises IPv6 that isn't actually reachable, so `smtplib` tries an IPv6 route to
Gmail first, hangs, then fails. `src/email_sender.py` now forces IPv4-only DNS
resolution and uses a 15-second connect timeout with 2 retries, so sends are fast
and failures surface immediately instead of hanging.

...
def from_excel(path: str, sheet_name=0) -> pd.DataFrame:
    """Read raw data from a local .xlsx/.xls file. sheet_name=0 -> first sheet."""
    return pd.read_excel(path, sheet_name=sheet_name, engine="openpyxl" if str(path).endswith("x") else None)

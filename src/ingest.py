...
_resolve_table(), which protects against, so "any format" actually works:
...
def from_docx(path: str) -> pd.DataFrame:
    ...
    for table in document.tables:
        ...
        score = (len(rows), len(rows[0]) if rows else 0)
        if score > best_score:
            best_score = score
            best_rows = rows

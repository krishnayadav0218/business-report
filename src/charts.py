...
def region_bar_chart(region_df, filename="region_target_vs_collection.png"):
    """Grouped bar chart: Target vs Collection by region (falls back to Collection-only if no Target)."""
    _ensure_dir()
    ...
    if has_target:
        ...
        ax.bar(list(x), region_df["Collection"], width=0.5, color=TEAL)

    ax.set_xticks(list(x))
    ax.set_xticklabels(region_df["Region"])
    ax.set_ylabel("Amount (₹)")
    title = "Target vs Collection by Region" if has_target else "Collection by Region"

...
def generate_insights(kpis: dict, region_df: pd.DataFrame, trend_df: pd.DataFrame,
    ...
    recommendations = []
    if achievement < 100:
        recommendations.append("Review the pipeline and follow up on pending collections to close the gap to target.")
    if kpis.get("total_cost") and kpis["profit"] < 0:
        recommendations.append("Investigate cost drivers — expenses currently exceed collections for the period.")
    if kpis.get("wow_growth") is not None and kpis["wow_growth"] < 0:
        recommendations.append("Address the declining trend with renewed outreach or a targeted promotion.")
    if not region_df.empty and "Target" in region_df.columns:
        achieved = region_df.assign(pct=region_df["Collection"] / region_df["Target"] * 100)
        below = achieved[achieved["pct"] < 100]
        if not below.empty:
            recommendations.append(f"Prioritize support for underperforming regions: {', '.join(below['Region'].tolist())}.")
    if not sp_df.empty and len(sp_df) > 1:
        recommendations.append("Recognize top performers publicly and share their approach with the rest of the team.")
    if not recommendations:
        recommendations.append("Performance is on track — continue the current strategy and monitor the trend weekly.")

...
def build_report(kpis: dict, chart_paths: dict, output_path="output/Weekly_Business_Report.pptx",
    ...
    # ---- Slide 2: About This Report (client-facing scope & purpose) ----
    period_start = kpis.get("period_start")
    period_end = kpis.get("period_end")
    period_range = None
    if period_start is not None and period_end is not None:
        try:
            period_range = f"{period_start.strftime('%d %b %Y')} to {period_end.strftime('%d %b %Y')}"
        except AttributeError:
            period_range = None

    scope_bits = []
    if chart_paths.get("region"):
        scope_bits.append("regional performance")
    if chart_paths.get("trend"):
        scope_bits.append("day-by-day trends")
    if chart_paths.get("leaderboard"):
        scope_bits.append("individual team contribution")
    scope_text = ", ".join(scope_bits) if scope_bits else "overall performance"

    s2 = _blank_slide(prs)
    _slide_header(s2, "About This Report", "A clear, complete view of performance for this period")
    about_points = [
        (f"Covers the period from {period_range}, based on {kpis.get('record_count', 0):,} recorded transactions."
         if period_range else f"Covers this reporting period, based on {kpis.get('record_count', 0):,} recorded transactions."),
        f"Brings together collections against target, {scope_text}, and clear next steps in one place.",
        "Every figure is drawn directly from the underlying transaction records \u2014 nothing is estimated or manually adjusted.",
        "Built to give a transparent, at-a-glance view of where the business stands, and what to focus on next.",
    ]
    _bullet_list(s2, MARGIN, Inches(2.0), Inches(12.1), Inches(3.8),
                 about_points, size=17, line_spacing=1.6, gap_after=18)
    _footer(s2, next_page())
    _set_notes(s2, _speak(
        "Quick framing before the numbers.",
        f"This report covers {period_range}." if period_range else "This report covers the current reporting period.",
        "It's meant to give a complete, honest picture of performance \u2014 collections against target, "
        "where the strength and weak spots are, and what we should do about it.",
        "Every number on the following slides comes straight from the transaction data, so what you're "
        "seeing is an accurate reflection of the period, not an estimate."
    ))
